...
class CameraViewController: UIViewController, AVCaptureDataOutputSynchronizerDelegate {
    ...
    // For testing, trying to print an array of the depth of all pixels that have been pressed on
    public var touchedPixels = [String]()
    
    // Count frames
    public var frameCount = 0
    
    // Count seconds of data collection to format testing procedure
    public var secondsOfScanning = 0
    
    // Only save every nth frame
    public let saveFrameFrequency = 30
    
    public var savedFrames = Array<Array<Array<Float>>>()
    
    // Allows average pixelBuffer to be used globally
    public var averagePixelBuffer: CVPixelBuffer?
    
    // testing transfer of information in s egue
    private var testRender = "Start Render"
    
    // height and width of PixelBuffer
    public var bufferHeight = 0
    public var bufferWidth = 0
    ...
    // MARK: - stopScanButtonPressed
    @IBAction func stopScanButtonPressed(_ sender: UIButton) {
        session.stopRunning()
        
        print("SAVED FRAMES: " + String(self.savedFrames.count))
        
        var fullScan = self.getAverageOfFrames(frames: Array(savedFrames[0 ..< 10]))
        var leftCenteredScan = self.getAverageOfFrames(frames: Array(savedFrames[10 ..< 20]))
        var rightCenteredScan = self.getAverageOfFrames(frames: Array(savedFrames[20 ..< 30]))
        
//        print("Volume")
//        print(DepthSlice.calculateVolumeJanky(5))
        
        let imageAnalysis = DepthSlice(fullScan: fullScan, leftCenteredScan: leftCenteredScan, rightCenteredScan: rightCenteredScan)
        //let imageAnalysis = DepthSlice(depthArray: savedFrames[4])
        
//        for i in 0...(averageOfFrames.count-1) {
//            for j in 0...(averageOfFrames[0].count-1) {
//                print(String(averageOfFrames[i][j]) + ", ", terminator: "")
//            }
//        }
    }
    
    public func getAverageOfFrames(frames: Array<Array<Array<Float>>>) -> Array<Array<Float>> {
        
        var sum: Float = 0
        var average: Float = 0
        var numPointsCounted: Float = 0
        
        //Saving data in an array to allow for manipulation and printing to console
        
        var averageOfFrames = Array<Array>(repeating: Array<Float>(repeating: 0, count: frames[0][0].count), count: frames[0].count)
        
        for row in 0...(frames[0].count - 1) {
            for col in 0...(frames[0][0].count - 1) {
                sum = 0
                numPointsCounted = 0
                for frame in 0...(frames.count - 1) {
                    if (!(frames[frame][row][col].isNaN)) {
                        sum += frames[frame][row][col]
                        numPointsCounted += 1
                    }
                }
                if (numPointsCounted > 0) {
                    average = Float(sum)/Float(numPointsCounted)
                } else {
                    average = Float(0)
                }
                averageOfFrames[row][col] = average
            }
        }
        
        return averageOfFrames
    }
...
//    }
    ...
    @IBAction func renderScanButtonPressed(_ sender: Any) {
        
       performSegue(withIdentifier: "goToRender", sender: self)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        vc.titleText = self.testRender
        
//        //Creating pixel buffer to store data in order to render
//        CVPixelBufferCreate(kCFAllocatorDefault, bufferWidth, bufferHeight, kCVPixelFormatType_DepthFloat32, nil, &averagePixelBuffer)
//
//        vc.pixelBufferHeight = savedFrames[0].count
//        vc.pixelBufferWidth = savedFrames[0][0].count
//        vc.pixelBufferBytesPerRow = CVPixelBufferGetBytesPerRow(averagePixelBuffer!)
//        CVPixelBufferLockBaseAddress(averagePixelBuffer!, .readOnly)
//        vc.finalPixelBufferAddress = CVPixelBufferGetBaseAddress(averagePixelBuffer!)
//        CVPixelBufferUnlockBaseAddress(averagePixelBuffer!, .readOnly)
//        print("override performed")
        ...
        if JETEnabled {
            ...
            dataOutputQueue.async {
                ...
                if (self.frameCount == 0) {
                    print("5 Seconds to Start of Data Collection. Align camera with center of breasts")
                    print("")
                }
                //test for saving pixel buffer
                if (self.frameCount % self.saveFrameFrequency == 0) {
                    
                    // 10 second/ frame scan of both breasts
                    if(self.secondsOfScanning >= 5 && self.secondsOfScanning < 15) {
                        self.saveFrameDepthData(depthFrame: depthPixelBuffer, videoFrame: videoPixelBuffer)
                        print("SAVING FRAME... " + String(self.savedFrames.count))
                    } else if (self.secondsOfScanning == 15) {
                        print("")
                        print("Move camera to center of left breast")
                        print("")
                    } else if (self.secondsOfScanning == 25) {
                        print("")
                        print("5 Seconds to Data Collection. Make sure phone is centered on left breast")
                        print("")
                    }
                        
                    // 10 second/ frame scan of left breast only
                    else if (self.secondsOfScanning >= 30 && self.secondsOfScanning < 40) {
                        self.saveFrameDepthData(depthFrame: depthPixelBuffer, videoFrame: videoPixelBuffer)
                        print("SAVING FRAME... " + String(self.savedFrames.count))
                    } else if (self.secondsOfScanning == 40) {
                        print("")
                        print("Move camera to center of right breast")
                        print("")
                    }
                    else if (self.secondsOfScanning == 50) {
                        print("")
                        print("5 Seconds to Data Collection. Make sure phone is centered on right breast")
                        print("")
                    }
                        
                    // 10 second/ frame scan of right breast only
                    else if (self.secondsOfScanning >= 55 && self.secondsOfScanning < 65) {
                        self.saveFrameDepthData(depthFrame: depthPixelBuffer, videoFrame: videoPixelBuffer)
                        print("SAVING FRAME... " + String(self.savedFrames.count))
                    }
                    //print("SECONDS OF SCANNING: " + String(self.secondsOfScanning))
                    self.secondsOfScanning += 1
                }
                //print("FRAME COUNT: " + String(self.frameCount))
                self.frameCount += 1
            }
        } else {
            // point cloud
    ...
    // MARK: - saveFrameDepthData
    //Loops through all the pixels in the pixel buffer and creates an array of all the pixel depths
    func saveFrameDepthData(depthFrame: CVPixelBuffer, videoFrame: CVPixelBuffer) {
        bufferHeight = CVPixelBufferGetHeight(depthFrame)
        bufferWidth = CVPixelBufferGetWidth(depthFrame)
        
        //initialize array to store depth values
        var frame = Array<Array>(repeating: Array<Float>(repeating: 0.0, count: bufferWidth), count: bufferHeight)
        
        for row in 0...(bufferHeight-1) {
            // Confirm depth data format
            assert(kCVPixelFormatType_DepthFloat16 == CVPixelBufferGetPixelFormatType(depthFrame))

            // Locks base memory address of the pixel buffer to make sure it is accessible
            CVPixelBufferLockBaseAddress(depthFrame, .readOnly)
            
            // Retrieve all data from memory in the desired row of the buffer
            let rowData = CVPixelBufferGetBaseAddress(depthFrame)! + row * CVPixelBufferGetBytesPerRow(depthFrame)
            
             let data = UnsafeMutableBufferPointer<Float32>(start: rowData.assumingMemoryBound(to: Float32.self), count: bufferWidth)
            
            for col in 0...(bufferWidth-1) {
                
                var f32Pixel = Float(0.0)
                var src = vImage_Buffer(data: &(data[col]), height: 1, width: 1, rowBytes: 2)
                var dst = vImage_Buffer(data: &f32Pixel, height: 1, width: 1, rowBytes: 4)
                vImageConvert_Planar16FtoPlanarF(&src, &dst, 0)
                frame[row][col] = f32Pixel * 100
                
            }
            
        }
        // Unlock address after processing data
        CVPixelBufferUnlockBaseAddress(depthFrame, .readOnly)
        
        // Add completed frame to global list of all frames that have been recorded during the session
    ...
    func updateDepthLabel(depthFrame: CVPixelBuffer, videoFrame: CVPixelBuffer) {
        ...
        if touchDetected {
            ...
            var f32Pixel = getPixelDepth(depthFrame: depthFrame, videoFrame: videoFrame, point: texturePoint)
            
            // Convert the depth frame format to cm
            let depthString = String(format: "%.2f cm", f32Pixel * 100)
            //touchedPixels.append(depthString)
    ...
    // Code taken from updateDepthLabel function, scales coordinates of pixel of interests and returns it's depth
    // Seperated this into it's own function because I also needed to use it in saveFrameDepthData and I didn't want to repeat code
    func getPixelDepth(depthFrame: CVPixelBuffer, videoFrame: CVPixelBuffer, point: CGPoint?) -> Float
    {
        guard let unwrappedPoint = point
            else {
                return 0.0
        }
        
        // scale
        // Seems like scale is always equal to 1 because the depthFrame and videoFrame are always the same size, so not sure this line was included
        let scale = CGFloat(CVPixelBufferGetWidth(depthFrame)) / CGFloat(CVPixelBufferGetWidth(videoFrame))

        // Not sure why x value is adjusted to be taken from the end of the depth frame instead of the start, might want to look into that more
        let depthPoint = CGPoint(x: CGFloat(CVPixelBufferGetWidth(depthFrame)) - 1.0 - unwrappedPoint.x * scale, y: unwrappedPoint.y * scale)
